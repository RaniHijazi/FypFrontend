...
export default function Messages({ navigation }) {
  ...
  const fetchUserMessages = async (userId) => {
    try {
      const response = await fetch(`http://192.168.1.141:7210/api/Message/${userId}`);
      if (!response.ok) {
  ...
  const fetchUserProfiles = async (messages, userId) => {
    ...
    for (const id of userIds) {
      try {
        const response = await fetch(`http://192.168.1.141:7210/api/User/${id}/profile`);
        if (!response.ok) {
  ...
  const fetchChatRooms = async () => {
    try {
      ...
      if (storedUserId !== null) {
        const userIdInt = parseInt(storedUserId, 10);
        const response = await fetch(`http://192.168.1.141:7210/api/ChatRoom/user/${userIdInt}`);
