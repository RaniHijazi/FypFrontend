...
export default function ChatScreen({ route }) {
  ...
  const fetchMessages = async (senderId, receiverId) => {
    try {
      const response = await fetch(`http://192.168.0.100:7210/api/Message/sender/${senderId}?receiverId=${receiverId}`);
      if (!response.ok) {
  ...
  const startSignalRConnection = async () => {
    if (!signalRConnection) {
      ...
      signalRConnection = new HubConnectionBuilder()
        .withUrl(`http://192.168.0.100:7210/chatHub?userId=${userId}`)
  ...
  const sendMessage = async () => {
    ...
    try {
      const url = `http://192.168.0.100:7210/api/Message/send?senderId=${userId}&recipientId=${data.id}&messageContent=${encodeURIComponent(chat)}`;
